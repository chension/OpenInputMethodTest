...
public class InputModeSwitcher {
    ...
    private static final int INPUT_TYPE_PINYIN = 3;//中文键盘
    ...
    static final         int TOGGLE_KEYCODE_PAGE_1 = 101;
    ...
    public void prepareToggleStates(SoftKey softKey) {
        ...
        if (mToggleStates.mQwerty && (mInputMode == INPUT_TYPE_ALL_EN) && (userKeyCode == 0)) { // 初始化英文键盘大小写.
            ...
            mInputMode = INPUT_TYPE_ALL_EN;
        } else if (mToggleStates.mQwerty && MODE_SKB_PINYIN == userKeyCode) {
            //中文模式 新增:::by chankey
    ...
    class ToggleStates {
        int mPageState = TOGGLE_KEYCODE_PAGE_1;//翻页
        boolean mSwitchSkb;
        boolean mQwerty;//是否是标准键盘
        boolean mQwertyUpperCase;//是否是标准键盘大写
        boolean mQwertyPinyin;
        List<Integer> mKeyStates = new ArrayList<Integer>();
    }
    ...
    public boolean isChineseText() {
        return mToggleStates.mQwertyPinyin;
    }
}
